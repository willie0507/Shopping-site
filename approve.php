...
if(isset($_GET['goods_id'])&&isset($_GET['goods_name'])&&isset($_GET['goods_price'])&&isset($_GET['goods_image'])&&isset($_GET['goods_date_added'])&&isset($_GET['in_stock'])){
	...
	echo '<strong>欲上架的商品不存在</strong>' ;
}

if(isset($_POST['submit'])){
	if($_POST['confirm'] == 'Yes'){
		$dbc = mysqli_connect('localhost', 'root', 'f1234568', 'shopping_website') ;
		$query = "UPDATE goods SET goods_active = 1 WHERE goods_id = $goods_id" ;
		mysqli_query($dbc, $query) ;
		$query = "UPDATE goods SET goods_date_approved = now() WHERE goods_id = $goods_id" ;
		mysqli_query($dbc, $query) ;
		mysqli_close($dbc) ;
		
		echo '<strong>商品已完成上架</strong>' ;
	}
	
	else{
		echo '<strong>你已取消上架作業</strong>' ;
	}
}
	...
	echo '<input type="submit" value="上架" name="submit">' ;
